...
function ConsultaActivaInner() {
  ...
  async function applyVoiceInstruction(section: VoiceTarget, dictado: string) {
    ...
    try {
      const result = await adjustNoteWithAssistant({
        encounter_id: encounterId,
        instruction: `En la sección "${section.label}", aplica esta instrucción dictada por el médico: "${intencion.instruccion}". Modifica únicamente lo necesario para cumplirla y conserva el resto de la nota.`,
