...
import { ASSIGNABLE_ROLES, isAssignableRole } from "@/lib/superadmin/roles";
      ...
      <div className="mt-5 space-y-5">
        ...
        <Card>
          <h2 className="flex items-center gap-2 text-sm font-semibold text-deep">
            <UserPlus size={16} /> Agregar profesional a tu institución
          </h2>
          <form
        ...
        <div className="overflow-hidden rounded-[14px] border border-line bg-surface">
          <div className="hidden grid-cols-[1.6fr_1fr_auto] gap-4 border-b border-line px-5 py-3 text-xs font-semibold uppercase tracking-wide text-muted sm:grid">
            <span>Usuario</span>
            <span>Rol</span>
            <span>Estado</span>
          </div>
          ...
          {visibles.length === 0 ? (
            ...
            visibles.map((user, index) => {
              ...
              const editable = isAssignableRole(user.role) && !deBaja;

              return (
                <div
                  key={user.id}
                  className={`grid grid-cols-1 gap-3 px-5 py-4 sm:grid-cols-[1.6fr_1fr_auto] sm:items-center sm:gap-4 ${
                    index ? "border-t border-line" : ""
                  } ${deBaja ? "bg-ice-soft/60" : ""}`}
                >
