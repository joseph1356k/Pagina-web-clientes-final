...
export function ClinicalOnboardingForm({ fullName }: { fullName: string }) {
  const [professionalType, setProfessionalType] = useState<
    "medico_general" | "medico_especialista" | "patologo"
  >("medico_general");
    ...
    <form action={action} className="mt-8 space-y-6">
      ...
      <div className="rounded-lg border border-line bg-surface p-5 shadow-[var(--shadow-sm)]">
        ...
        <fieldset className="mt-5 grid gap-3 sm:grid-cols-3">
          ...
          <label className={`cursor-pointer rounded-md border p-4 transition-colors ${professionalType === "patologo" ? "border-accent bg-ice-soft ring-1 ring-accent/20" : "border-line hover:border-mist"}`}>
            <input
              className="sr-only"
              type="radio"
              name="professionalType"
              value="patologo"
              checked={professionalType === "patologo"}
              onChange={() => {
                setProfessionalType("patologo");
                setSpecialtyCode("patologia");
              }}
            />
            <span className="block text-sm font-semibold text-deep">Patólogo/a</span>
            <span className="mt-1 block text-xs text-muted">Informes de patología: histopatología, citología e inmunohistoquímica. Habilita generar informes desde una foto de la hoja.</span>
          </label>
        </fieldset>
